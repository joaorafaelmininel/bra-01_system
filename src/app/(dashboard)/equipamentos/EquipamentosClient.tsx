...
export default function EquipamentosClient({ equipment, sections }: Props) {
  ...
  // ── pré-filtro pelo estado global da topbar ────────────────────────────────
  const byEstado = useMemo(() =>
    estado === 'Todos'
      ? equipment
      : equipment.filter(e => e.proprietario === estado),
    [equipment, estado]
  )

  // ── filtros locais sobre byEstado ──────────────────────────────────────────
  const filtered = useMemo(() => byEstado.filter(e => {
  ...
  }), [byEstado, search, filterSec, filterStatus])
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#5A6478', marginTop: 2, letterSpacing: '0.08em' }}>
            CACHE BRA-01 · PADRÃO INSARAG/FEMA
          </div>
        </div>
      ...
      {/* Stats — calculados sobre byEstado */}
      <Stats equipment={byEstado} />

      {/* Seções — chips calculados sobre byEstado */}
      <SectionBreakdown equipment={byEstado} sections={sections} filter={filterSec} onFilter={setFilterSec} />
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} style={{ padding: '40px 14px', textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: 11, color: '#5A6478' }}>
                  {byEstado.length === 0
                    ? estado !== 'Todos'
                      ? `Nenhum equipamento registrado para ${estado}.`
                      : 'Nenhum equipamento cadastrado. Clique em "+ Novo item" para começar.'
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: '10px 14px', maxWidth: 300 }}>
                    <div style={{ color: '#E8EDF5', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {e.nome}
                    </div>
                  ...
                  {/* Seção / Grupo */}
                  <td style={{ padding: '10px 14px' }}>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: secColor, fontWeight: 600 }}>
                      {e.section?.codigo} — {e.section?.nome_pt}
                    </div>
                    {e.group && (
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#5A6478', marginTop: 1 }}>
                        {e.group.nome}
                  ...
                  <td style={{ padding: '10px 14px', textAlign: 'right' }}>
                    <Link
                      ...
                      style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#5A6478', textDecoration: 'none', letterSpacing: '0.05em', transition: 'color .15s' }}
