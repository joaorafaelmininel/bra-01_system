...
type FormData = {
  nome_completo: string; cpf: string; nome_guerra: string; data_nascimento: string; sexo: string
  peso_kg: string; altura_cm: string; tipo_sanguineo: string
  tipo_membro: string; instituicao: string; estado_base: string; posto_graduacao: string
  registro_funcional: string; registro_geral: string; numero_cedula: string
  data_ingresso_instituicao: string; tempo_aposentadoria_anos: string
  cargo_funcao: string; registro_profissional: string; tempo_servico_anos: string
  status_operacional: string; aptidao_operacional: string
  passaporte_numero: string; passaporte_validade: string
  telefone_celular: string; telefone_funcional: string; email_pessoal: string; email_institucional: string
  contato_emergencia_nome: string; contato_emergencia_telefone: string; contato_emergencia_relacao: string
  componente_primario: string; componente_secundario: string
}

const INITIAL: FormData = {
  nome_completo: '', cpf: '', nome_guerra: '', data_nascimento: '', sexo: 'Masculino',
  peso_kg: '', altura_cm: '', tipo_sanguineo: 'O+',
  tipo_membro: 'militar', instituicao: 'CBPMESP', estado_base: 'Central', posto_graduacao: 'Capitão',
  registro_funcional: '', registro_geral: '', numero_cedula: '',
  data_ingresso_instituicao: '', tempo_aposentadoria_anos: '',
  cargo_funcao: '', registro_profissional: '', tempo_servico_anos: '',
  status_operacional: 'Ativo', aptidao_operacional: 'Apto',
  passaporte_numero: '', passaporte_validade: '',
  telefone_celular: '', telefone_funcional: '', email_pessoal: '', email_institucional: '',
  contato_emergencia_nome: '', contato_emergencia_telefone: '', contato_emergencia_relacao: '',
  componente_primario: 'Rescue', componente_secundario: '',
}
...
function SectionTitle({ num, children }: { num: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
      ...
      <span style={{ fontFamily: 'var(--font-cond)', fontSize: 13, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#9BA8BC' }}>
        {children}
      </span>
...
export default function NovoMembroForm() {
  ...
  const sectionStyle: React.CSSProperties = {
    ...
    borderRadius: 5, padding: '20px 20px',
  }
  ...
  return (
    <div style={{ maxWidth: 880, margin: '0 auto', padding: '24px 24px 60px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div>
          <div style={{ fontFamily: 'var(--font-cond)', fontSize: 22, fontWeight: 800, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#fff' }}>Novo membro</div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: '#5A6478', letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 2 }}>Cadastro no efetivo BRA-01</div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 14 }}>
            <Field label="Nome completo" required>
              <input style={inputStyle} value={form.nome_completo} onChange={set('nome_completo')} required placeholder="Nome completo conforme documentação" />
            </Field>
          </div>
          <div style={{ ...grid3, marginTop: 14 }}>
            <Field label="CPF" required>
              <input style={inputStyle} value={form.cpf} onChange={set('cpf')} required placeholder="000.000.000-00" />
            </Field>
            <Field label="Nome de guerra">
              <input style={inputStyle} value={form.nome_guerra} onChange={set('nome_guerra')} placeholder="Ex: Mininel" />
            </Field>
        ...
        <div style={sectionStyle}>
          ...
          <div style={grid3}>
            <Field label="Peso (kg)" required>
              <input type="number" step="0.1" min="30" max="200" style={inputStyle} value={form.peso_kg} onChange={set('peso_kg')} required placeholder="75.0" />
            </Field>
            <Field label="Altura (cm)" required>
              <input type="number" min="100" max="250" style={inputStyle} value={form.altura_cm} onChange={set('altura_cm')} required placeholder="175" />
            </Field>
        ...
        <div style={sectionStyle}>
          ...
          <div style={grid2}>
            ...
            <Field label="Registro funcional">
              <input style={inputStyle} value={form.registro_funcional} onChange={set('registro_funcional')} placeholder="Nº de matrícula" />
            </Field>
            <Field label="Registro geral (RG)">
              <input style={inputStyle} value={form.registro_geral} onChange={set('registro_geral')} placeholder="RG" />
            </Field>
            <Field label="Número da cédula">
              <input style={inputStyle} value={form.numero_cedula} onChange={set('numero_cedula')} placeholder="Cédula de identidade funcional" />
            </Field>
            ...
            <Field label="Tempo de serviço (anos)">
              <input type="number" min="0" style={inputStyle} value={form.tempo_servico_anos} onChange={set('tempo_servico_anos')} placeholder="Ex: 15" />
            </Field>
            <Field label="Tempo para aposentadoria (anos)">
              <input type="number" min="0" style={inputStyle} value={form.tempo_aposentadoria_anos} onChange={set('tempo_aposentadoria_anos')} placeholder="Ex: 10" />
            </Field>
            <Field label="Cargo / Função atual">
              <input style={inputStyle} value={form.cargo_funcao} onChange={set('cargo_funcao')} placeholder="Ex: Comandante de Seção" />
            </Field>
            <Field label="Registro profissional">
              <input style={inputStyle} value={form.registro_profissional} onChange={set('registro_profissional')} placeholder="CREA, CRM, CRO..." />
            </Field>
          </div>
        </div>
        ...
        <div style={sectionStyle}>
          ...
          <div style={grid2}>
            <Field label="Número do passaporte" hint="Deixe em branco se não possuir">
              <input style={inputStyle} value={form.passaporte_numero} onChange={set('passaporte_numero')} placeholder="Ex: BR123456" />
            </Field>
          ...
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: '#2E3848', marginTop: 10 }}>
            Upload de documentos disponível no perfil do membro após o cadastro.
          </p>
        </div>
        ...
        <div style={sectionStyle}>
          ...
          <div style={grid2}>
            <Field label="Telefone celular" required>
              <input style={inputStyle} value={form.telefone_celular} onChange={set('telefone_celular')} required placeholder="(11) 99999-9999" />
            </Field>
            <Field label="Telefone funcional">
              <input style={inputStyle} value={form.telefone_funcional} onChange={set('telefone_funcional')} placeholder="(11) 3333-3333" />
            </Field>
            <Field label="E-mail pessoal" required>
              <input type="email" style={inputStyle} value={form.email_pessoal} onChange={set('email_pessoal')} required placeholder="nome@email.com" />
            </Field>
            <Field label="E-mail institucional">
              <input type="email" style={inputStyle} value={form.email_institucional} onChange={set('email_institucional')} placeholder="nome@bombeiros.gov.br" />
            </Field>
          </div>
        </div>
        ...
        <div style={sectionStyle}>
          ...
          <div style={grid3}>
            <Field label="Nome" required>
              <input style={inputStyle} value={form.contato_emergencia_nome} onChange={set('contato_emergencia_nome')} required placeholder="Nome completo" />
            </Field>
            <Field label="Telefone" required>
              <input style={inputStyle} value={form.contato_emergencia_telefone} onChange={set('contato_emergencia_telefone')} required placeholder="(11) 99999-9999" />
            </Field>
            <Field label="Relação" required>
              <input style={inputStyle} value={form.contato_emergencia_relacao} onChange={set('contato_emergencia_relacao')} required placeholder="Ex: Cônjuge, Pai, Mãe" />
            </Field>
          </div>
        </div>
