...
// ── tipos e constantes omitidos por brevidade — mantém os do arquivo original ──
// Cole abaixo do import, mantendo todo o resto do seu EditarMembroForm atual
// e adicione apenas o bloco do botão de exclusão no final do formulário:

// ─── Botão de exclusão — adicionar no final do <form>, após o botão SALVAR ───

export function BotaoExcluirMembro({ membroId, nomeGuerra }: { membroId: string; nomeGuerra: string }) {
  ...
  async function handleDelete() {
    ...
    const { error } = await supabase.from('members').delete().eq('id', membroId)
    if (error) {
