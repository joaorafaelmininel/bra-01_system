...
export default function MembrosClient({ members }: Props) {
  ...
  // ── membros pré-filtrados pelo estado global da topbar ─────────────────────
  ...
  // ── filtros locais aplicados em cima do byEstado ───────────────────────────
  const filtered = useMemo(() => byEstado.filter(m => {
    const q = search.toLowerCase()
    const matchSearch = !q
      || m.nome_completo.toLowerCase().includes(q)
      || (m.nome_guerra?.toLowerCase().includes(q) ?? false)
      || (m.posto_graduacao?.toLowerCase().includes(q) ?? false)
    return matchSearch
  ...
  }), [byEstado, search, filterStatus, filterComp, filterInst])

  // ── stats calculados sobre byEstado (respeita filtro global) ──────────────
  const total  = byEstado.length
  const ativos = byEstado.filter(m => m.status_operacional === 'Ativo').length
  const aptos  = byEstado.filter(m => m.aptidao_operacional === 'Apto').length
  const semPP  = byEstado.filter(m =>
    !m.passaporte_validade || new Date(m.passaporte_validade) < new Date()
  ).length
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
        ...
        {/* Select de instituição só aparece quando estado = Todos */}
        {estado === 'Todos' && (
      ...
      <ComponentBreakdown members={byEstado} filter={filterComp} onFilter={setFC} />
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              return (
                <tr
                  key={m.id}
                  style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: '#0D1117', transition: 'background .1s' }}
                ...
                >
                  ...
                  <td style={{ padding: '10px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: apt ? '#00A550' : '#FF6B6B' }}>
                        {m.aptidao_operacional}
                      </span>
                    </div>
                  </td>
                  ...
                  <td style={{ padding: '10px 14px', textAlign: 'right' }}>
                    <Link
                      ...
                      style={{
                        ...
                        textDecoration: 'none', transition: 'color .15s', letterSpacing: '0.05em',
                      }}
