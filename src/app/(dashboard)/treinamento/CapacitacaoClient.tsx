...
function TabVisaoGeral({ members, qualifications, courseCatalog }: {
  members: Member[]; qualifications: Qual[]; courseCatalog: Course[]
}) {
  const mandatories = courseCatalog.filter(c => c.obrigatorio_heavy)

  const totalQuals = qualifications.length
  const vencidas   = qualifications.filter(q =>
    q.data_validade && new Date(q.data_validade) < new Date()
  ).length
      ...
      {mandatories.length > 0 && (
        <div style={{ background: '#131920', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 5, overflow: 'hidden' }}>
          <div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: 'var(--font-cond)', fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#9BA8BC' }}>
              Cobertura — Cursos obrigatórios INSARAG
            </span>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
              ...
              <tbody>
                {members.map(m => {
                  ...
                  const covered = mandatories.filter(c =>
                    memberQuals.some(q =>
          ...
          <div style={{ padding: '8px 14px', borderTop: '1px solid rgba(255,255,255,0.07)', display: 'flex', gap: 16 }}>
            {[
              { c: '#00A550', l: 'Válida' },
              { c: '#FFDF00', l: 'Vence em 60–180d' },
              { c: '#E87722', l: 'Vence em 60d' },
              { c: '#FF6B6B', l: 'Vencida' },
              { c: '#2E3848', l: 'Não possui' },
            ].map(({ c, l }) => (
...
function TabQualificacoes({ qualifications, members }: { qualifications: Qual[]; members: Member[] }) {
  ...
  const filtered = useMemo(() => qualifications.filter(q => {
    ...
    const match = !txt ||
      q.nome_curso?.toLowerCase().includes(txt) ||
      q.codigo_curso?.toLowerCase().includes(txt) ||
      q.entidade_emissora?.toLowerCase().includes(txt)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as const, alignItems: 'center' }}>
        ...
        <select value={filterMember} onChange={e => setFM(e.target.value)} style={selectStyle}>
          ...
          {members.map(m => (
            <option key={m.id} value={m.id}>{m.nome_guerra ?? m.nome_completo.split(' ')[0]}</option>
          ))}
        </select>
      ...
      <div style={{ border: '1px solid rgba(255,255,255,0.07)', borderRadius: 5, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={6} style={{ padding: '40px', textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: 11, color: '#5A6478' }}>
...
function TabAdicional({ additionalTraining, members }: { additionalTraining: Training[]; members: Member[] }) {
  ...
  const filtered = useMemo(() => additionalTraining.filter(t => {
    ...
    const m = !q ||
      ...
      t.organizador?.toLowerCase().includes(q) ||
...
function TabIdiomas({ languages, members, usarFunctionsRequiringEnglish }: {
  languages: Language[]; members: Member[]; usarFunctionsRequiringEnglish: UsarFn[]
}) {
  const membersWithEnglish = members.filter(m =>
    languages.some(l =>
      l.member_id === m.id &&
      l.idioma.toLowerCase().includes('ingl') &&
      ['Intermediário', 'Avançado', 'Fluente'].includes(l.nivel)
    )
  )
  ...
  languages.forEach(l => {
    if (!idiomasMap[l.idioma]) idiomasMap[l.idioma] = { count: 0, members: [] }
    idiomasMap[l.idioma].count++
    const name = l.member?.nome_guerra ?? l.member?.nome_completo?.split(' ')[0]
    if (name) idiomasMap[l.idioma].members.push(name)
  })
...
export default function CapacitacaoClient({
  members,
  qualifications,
  additionalTraining,
  languages,
  courseCatalog,
  usarFunctionsRequiringEnglish,
}: Props) {
  const { estado } = useEstado()
  const [activeTab, setActiveTab] = useState('Visão Geral')

  // ── Filtro global: membros do estado selecionado ───────────────────────────
  const filteredMembers = useMemo(() =>
    estado === 'Todos'
      ? members
      : members.filter(m => m.instituicao === estado),
    [members, estado]
  )

  // ── Cascata: dados derivados filtrados pelos member_ids resultantes ────────
  const memberIds = useMemo(() => new Set(filteredMembers.map(m => m.id)), [filteredMembers])

  const filteredQuals    = useMemo(() => qualifications.filter(q => memberIds.has(q.member_id)),     [qualifications, memberIds])
  const filteredTraining = useMemo(() => additionalTraining.filter(t => memberIds.has(t.member_id)), [additionalTraining, memberIds])
  const filteredLangs    = useMemo(() => languages.filter(l => memberIds.has(l.member_id)),          [languages, memberIds])

  // ── Alertas calculados sobre dados filtrados ───────────────────────────────
  const vencidas = filteredQuals.filter(q =>
    q.data_validade && new Date(q.data_validade) < new Date()
  ).length
  const vencendo = filteredQuals.filter(q => {
    if (!q.data_validade) return false
    const d = Math.ceil((new Date(q.data_validade).getTime() - Date.now()) / 86400000)
    return d >= 0 && d < 60
  }).length
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ fontFamily: 'var(--font-cond)', fontSize: 22, fontWeight: 800, letterSpacing: '0.06em', textTransform: 'uppercase' as const, color: '#fff' }}>
              Capacitação / Especializações
            </div>
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#5A6478', marginTop: 2, letterSpacing: '0.08em' }}>
            QUALIFICAÇÕES · TREINAMENTOS · IDIOMAS · BRA-01
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 2, borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        {TABS.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{
            ...
            {tab}
            {tab === 'Qualificações' && filteredQuals.length > 0 && (
              <span style={{ marginLeft: 6, fontFamily: 'var(--font-mono)', fontSize: 9, padding: '0 4px', borderRadius: 2, background: 'rgba(0,158,219,0.15)', color: '#009EDB' }}>
                {filteredQuals.length}
              </span>
            )}
            {tab === 'Capacitação Adicional' && filteredTraining.length > 0 && (
              <span style={{ marginLeft: 6, fontFamily: 'var(--font-mono)', fontSize: 9, padding: '0 4px', borderRadius: 2, background: 'rgba(0,158,219,0.15)', color: '#009EDB' }}>
                {filteredTraining.length}
              </span>
            )}
            {tab === 'Idiomas' && filteredLangs.length > 0 && (
              <span style={{ marginLeft: 6, fontFamily: 'var(--font-mono)', fontSize: 9, padding: '0 4px', borderRadius: 2, background: 'rgba(0,158,219,0.15)', color: '#009EDB' }}>
      ...
      {/* Conteúdo — todos os dados já pré-filtrados pelo estado */}
      {activeTab === 'Visão Geral'           && <TabVisaoGeral members={filteredMembers} qualifications={filteredQuals} courseCatalog={courseCatalog} />}
      {activeTab === 'Qualificações'          && <TabQualificacoes qualifications={filteredQuals} members={filteredMembers} />}
      {activeTab === 'Capacitação Adicional'  && <TabAdicional additionalTraining={filteredTraining} members={filteredMembers} />}
      {activeTab === 'Idiomas'                && <TabIdiomas languages={filteredLangs} members={filteredMembers} usarFunctionsRequiringEnglish={usarFunctionsRequiringEnglish} />}
    </div>
  )
}
